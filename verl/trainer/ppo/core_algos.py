...
@register_adv_est(AdvantageEstimator.GRPO_PNSR)  # or simply: @register_adv_est("grpo_pnsr")
def compute_grpo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    ...
    Same as vanilla GRPO advantage estimation, except that positive and negative advantage are then weighted individually. 
    """
    scores = token_level_rewards.sum(dim=-1)

    psr_weight = config.psr_weight if config is not None else 1.0
    nsr_weight = config.nsr_weight if config is not None else 1.0
    ...
    with torch.no_grad():
        ...
        # PNSR scaling:
        # positive advantage -> PSR part
        # negative advantage -> NSR part
        scores = torch.where(
            scores > 0,
            scores * psr_weight,
            scores * nsr_weight,
