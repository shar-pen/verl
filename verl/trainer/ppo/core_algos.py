...
@register_adv_est(AdvantageEstimator.GRPO_PNSR)
def compute_grpo_pnsr_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    """Compute GRPO advantages with separate positive and negative scaling."""
    scores = token_level_rewards.sum(dim=-1)
